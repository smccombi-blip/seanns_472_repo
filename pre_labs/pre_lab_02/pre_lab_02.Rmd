...
**ANSWER:** The code is showing the percentage of votes that were cast on election day compared to the total number of votes. 
...
**ANSWER:** The results are arranged in descending order by percentage. It also selects which columns I want to view in the data table, eliminating the rest and making the data easier to view. 
...
**ANSWER:** I do find this to be more interesting because I can view which candidates do not receive election day votes right away compared to the total. 
...
standardized_maryland_cities <- maryland_cities |>
  mutate(
    # named upper_city, which has the valeus from the "city " column but in uppercase instead
...
**ANSWER:** str_to_title() capitalizes the first letter of every word. str_to_sentence() capitalizes the first letter of each sentence. str_to_lower() converts to lowercase. 
...
**ANSWER:** The mutate statement is adding a new amount_category column that places the contribution amount into a specific range, rather than sticking with the specific amount. For example, I can see all the donations that are "Less than $100."
...
**ANSWER:** Less than $100 has the most contributions. 
...
**ANSWER:** Now, we can view all the votes that are specifically for a U.S. Senator from Prince George's County.
...
and_prince_georges <- primary_24 |>
    # give me the rows where the county name is Prince George's County OR where the office name is U.S. Senator
...
**ANSWER:** The dates are better formatted to look like a real calendar date. 
...
**ANSWER:** The data is showing all the amounts and expenditures that were made during each individual month.
