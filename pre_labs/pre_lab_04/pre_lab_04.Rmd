...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are multiple variations of the same groups throughout the data table, making them all seperate entities. 
...
dc_food |>
  ...
  # str_detect: any values that contain the text specified in your code
  # ^w|^W "give me any values that START WITH (^) a lowercase w or an uppercase W"
...
cleaned_dc_food <- dc_food |>
  # part that's new: key_collision_merge() from refinr
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Answer** "Hilton Washington" is named as a recipient that starts with W due to its Washington name. 
