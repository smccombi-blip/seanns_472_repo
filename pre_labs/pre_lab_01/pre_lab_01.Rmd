...
**Answer:**It returns the number "2" in the Values section of the environment. 
...
**Answer** last_name is a chr column / data type. "amount" is a dbl column / data type. 
...
```{r}
# say the name of our data set, and then tell R we're going to do SOMETHING with / to it
maryland_winred_contributions |>
  # group together all the identical values in the date column, and then do SOMETHING else with the data
  group_by(date) |>
...
**Answer**The answer represents how many contributions were made on a specific date. 
...
maryland_winred_contributions |>
  ...
  # arrangeing the data in descending order (largest to smallest) by the values in count_contribs
  arrange(desc(count_contribs))
```

Compared to the previous codeblock, what has changed and how did that affect the result?

**Answer**We can now view which date received the most donations. 
...
**Answer**We can view how many contributors donated a specific amount. 
...
**Answer**We can view the total amount donated to each organization. 
...
**Answer**We can now view the mean and median amounts donated. The top result from Maryland is Hogan for Maryland Inc. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted? **Answer** We can now view the minimum and maximum amounts donated within each county. It is arranged by the total amounts of donations. 
